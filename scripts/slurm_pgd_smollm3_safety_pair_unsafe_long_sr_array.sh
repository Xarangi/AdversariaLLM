#!/bin/bash
#SBATCH --job-name=pgd-smollm3-safety-pair-unsafe
...
#SBATCH --array=0-99
#SBATCH --output=/home/mila/s/sarangis/AdversariaLLM/slurm-pgd-smollm3-safety-pair-unsafe-%A_%a.out
#SBATCH --error=/home/mila/s/sarangis/AdversariaLLM/slurm-pgd-smollm3-safety-pair-unsafe-%A_%a.err
...
EMBED_DIR="${SCRATCH_RUN_BASE}/embeddings/pgd-smollm3-unsafe/${SLURM_JOB_ID}_${SLURM_ARRAY_TASK_ID}"
mkdir -p "${SAVE_DIR}" "${EMBED_DIR}"

MONGOD_BIN=/home/mila/s/sarangis/AdversariaLLM/local/mongodb/bin/mongod
MONGO_PORT=$((36017 + SLURM_ARRAY_TASK_ID))
